...
allconfs=data.frame(matrix(ncol=3, nrow=0))
for (i in 1:dim(samps)[1]) {
    ...
    pdf(plotfile)
    print(ggplot(plotconf, aes(x=fpr, y=tpr, colour=mtase)) +
        ...
        theme_bw())
    dev.off()
    allconfs=rbind(allconfs, plotconf)
}

plotall=paste0('~/Dropbox/yfan/methylation/methbin/event/roc_all.pdf')
pdf(plotall)
print(ggplot(allconfs, aes(x=fpr, y=tpr, colour=mtase)) +
      geom_line(aes(linetype=samp)) +
      geom_abline(slope=1, intercept=0) +
      xlim(0,1) +
      ylim(0,1) +
