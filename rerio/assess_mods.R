...
plot_modprobs <- function(samp, motifs, base) {
    ...
    cols=c('read_name', 'read_index', 'read_base', 'ref_name','ref_index', 'ref_base', 'pAmod', 'pCmod')
    sampmodinfo=tibble(
    ...
    for (motif in motifs) {
        ...
        modinfo=read_csv(modfile, col_names=cols) %>%
            mutate(motif=motif) %>%
            mutate(samp=samp) %>%
            filter(read_base==base) %>%
            mutate(probAmod=as.numeric(pAmod)) %>%
            mutate(probCmod=as.numeric(pCmod))
...
plot_moderrors <- function(samp, motifs, base) {
    ...
    for (motif in motifs) {
        ...
        errinfo=read_csv(modfile, col_names=cols) %>%
            group_by(read_name) %>%
            summarise(numerr=sum(read_base!=base), errfrac=/length(read_base)) %>%
            mutate(motif=motif) %>%
            mutate(samp=samp)
...
plot=ggplot(allerr, aes(x=samp, y=errfrac, colour=motif, fill=motif, alpha=.3)) +
    ...
    ggtitle(samp) +
    xlab('Sample') +
    ylab('Fraction of motifs that are erroneous') +
    theme_bw()
print(plot)
dev.off()
